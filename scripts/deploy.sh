...
echo "$TIME_NOW > 현재 실행중인 Docker 컨테이너 pid 확인" >> /home/ubuntu/app/deploy.log
CURRENT_PID=$(sudo docker container ls -q)

if [ -z $CURRENT_PID ]
then
  echo "$TIME_NOW > 현재 구동중인 Docker 컨테이너가 없으므로 종료하지 않습니다." >> /home/ubuntu/app/deploy.log
else
  echo "$TIME_NOW > sudo docker stop $CURRENT_PID" # 현재 구동중인 Docker 컨테이너 있으면 모두 중지
  sleep 5
fi
